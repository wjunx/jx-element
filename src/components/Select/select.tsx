...
export const Select:FC<SelectProps> = (props) => {
  ...
  const containerClass = classNames('jx-select', {
  ...
  return (
    <div className={containerClass} ref={containerRef}>
      <div className="jx-select-input" onClick={handleClick}>
        <Input
          ref={input}
      ...
      <SelectContext.Provider value={passedContext}>
        <Transition
            ...
            timeout={300}
          >
          <ul className="jx-select-dropdown">
            {generateOptions()}
          </ul>
        </Transition>
      </SelectContext.Provider>
      {multiple &&
        <div className="jx-selected-tags" style={{maxWidth: containerWidth.current - 32}}> 
          {
            selectedValues.map((value, index) => {
              return (
                <span className="jx-tag" key={`tag-${index}`}>
                  {value}
